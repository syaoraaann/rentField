import { Layout, Button, Row, Col, Typography, Card } from "antd";
import { LeftOutlined, RightOutlined, PlusOutlined } from '@ant-design/icons';

import { useState, useEffect } from "react";
import { Link, useNavigate } from "react-router-dom";
import "../../pages/landing_page/index.css";

//Images
import rentfieldlogo from "../../assets/images/rflogo.png";
import users from "../../assets/images/users.png";
import soccer2 from "../../assets/images/soccer2.jpg";
import bultang from "../../assets/images/bultang.jpg";
import futsal from "../../assets/images/futsal.jpg";
import voli from "../../assets/images/voli.jpg";
import biliard from "../../assets/images/biliard.jpg";
import ourteam from "../../assets/images/ourteam.png";
import basket from "../../assets/images/basket.jpg";
import basket3 from "../../assets/images/basket3.jpg";
import Column from "antd/es/table/Column";

const { Title } = Typography;
const { Header, Footer } = Layout;
const { Meta } = Card;

function LandingPage() {
  const navigate = useNavigate();
  const [activeSection, setActiveSection] = useState("Home");
  const [hoveredIndex, setHoveredIndex] = useState(null);

  //Scroll to section
  const scrollToSection = (sectionId) => {
    const element = document.getElementById(sectionId);
    if (element) {
      element.scrollIntoView({ behavior: "smooth" });
      setActiveSection(sectionId);
    }
  };

  useEffect(() => {
    const handleScroll = () => {
      const sections = ["Home", "Services", "About", "Contact"];
      const scrollPosition = window.scrollY;

      for (const sectionId of sections) {
        const element = document.getElementById(sectionId);
        if (element) {
          const { offsetTop, offsetHeight } = element;
          if (
            scrollPosition >= offsetTop - 100 &&
            scrollPosition < offsetTop + offsetHeight - 100
          ) {
            setActiveSection(sectionId);
            break;
          }
        }
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  //Handle login button
  const handleLoginClick = () => {
    navigate("/login");
  };

  const cards = [
    {
      number: "01.",
      title: 'Effectiveness & Efficiency',
      content:
        'Provide effectiveness and efficiency for user who want to rent field or place.',
    },
    {
      number: "02.",
      title: 'Effectiveness & Efficiency',
      content:
        'Provide effectiveness and efficiency for user who want to rent field or place.',
    },
    {
      number: "03.",
      title: 'Provide Status Field',
      content:
        'We provide information about status field or place so user can check easily the field or place already booked or not.',
    },
  ];

  return (
    <Layout className="layout-signin layout-default">
      <Header>
        <div className="header-col header-brand">
          <img
            src={rentfieldlogo}
            alt="Rent Field Logo"
            className="logo-landing"
          />
        </div>
        <div className="header-col header-nav">
          <nav>
            <Link
              onClick={() => scrollToSection("Home")}
              className={`px-4 py-2 ${
                activeSection === "Home" ? "active-link" : "text-gray-600"
              }`}
            >
              Home
            </Link>
            <Link
              onClick={() => scrollToSection("Services")}
              className={`px-4 py-2 ${
                activeSection === "Services" ? "active-link" : "text-gray-600"
              }`}
            >
              Services
            </Link>
            <Link
              onClick={() => scrollToSection("About")}
              className={`px-4 py-2 ${
                activeSection === "About" ? "active-link" : "text-gray-600"
              }`}
            >
              About
            </Link>
            <Link
              onClick={() => scrollToSection("Contact")}
              className={`px-4 py-2 ${
                activeSection === "Contact" ? "active-link" : "text-gray-600"
              }`}
            >
              Contact
            </Link>
          </nav>
        </div>
        <div className="header-col header-btn">
          <Button type="primary" onClick={handleLoginClick}>
            Login
          </Button>
        </div>
      </Header>

      <main>
        <section
          id="Home"
          className="home-section"
        >
          <div className="div-wrapper content-animation">
            <div>
              <Col>
                <h2 className="h2-homesection" style={{color: "#ABFD13"}}>
                  Field Rental
                </h2>
                <h2 className="h2-homesection" style={{color: "#D9D9D9"}}>
                  Made Easier For You
                </h2>
              </Col>
            </div>
            <div>
              <Row>
                <img
                  src={users}
                  alt="Users"
                  style={{width: "200px", paddingRight: "20px"}}
                />
                <p style={{fontSize: "28px", alignContent: "center", paddingTop: "20px"}}>
                  6K
                  <PlusOutlined
                    style={{
                      fontSize: "28px",
                      marginLeft: "8px",
                      color: "white", // Warna bisa disesuaikan
                      fontWeight: "bold", 
                    }}
                  />
                </p>
              </Row>
            </div>
            <div>
              <p className="paragraph-home">
                Welcome to the most comprehensive field rental platform! Enjoy
                the convenience of finding and booking your favorite field for
                various sports such as futsal, basketball, tennis, and more.
              </p>
            </div>
          </div>
          <div className="content-animation" style={{ paddingTop: "15px"}}>
            <Button type="primary" onClick={handleLoginClick} className="getstarted-button">
              Get Started
            </Button>
          </div>
        </section>

        <section
          id="Services"
          className="services-section"
        >
          <Col className="content-animation">
           <Row style={{paddingBottom: "110px"}}>
            <div>
              <h2 className="h2-services">
                Our Services
                <span style={{ color: "#D9D9D9" }}><br />are the best choice</span>
              </h2>
            </div>
            <div>
              <p className="paragraph-services">
              Create an unforgettable sports experience with the best facilities available anytime according to your needs. Let's start now to find the ideal field for your next match!
              </p>
            </div>
           </Row>
           <div className="cards-container">
            {cards.map((card, index) => (
              <Card
                key={index}
                className={`cards ${hoveredIndex ? 'hovered' : ''}`}
                onMouseEnter={() => setHoveredIndex(null)}
                onMouseLeave={() => setHoveredIndex(null)}
              >
                <h3 className="cards-title">{card.number}</h3>
                <h3 className="cards-title">{card.title}</h3>
                <p className="cards-content">{card.content}</p>
              </Card>
            ))}
          </div>
          </Col>
        </section>

        <section
          id="About"
          className="about-section"
        >
          <Col>
            <Row className="content-animation">
              <div style={{paddingRight: "80px", paddingTop: "150px"}}>
                <h2 className="h2-about">
                  Vision &
                  <span style={{ color: "#D9D9D9" }}><br />Mision</span>
                </h2>
              </div>
              <Col style={{alignItems: "start"}}>
              <div style={{ marginBottom: "50px" }}>
                <Card 
                title={<h2 style={{ color: '#ABFD13',fontWeight: "bold" }}>Vision</h2>} 
                bordered= {false} 
                className="cards-about">
                  <p>Create an unforgettable sports experience with the best facilities available anytime according to your needs. Let's start now to find the ideal field for your next match!</p>
                </Card>
              </div>
              <div>
                <Card 
                title={<h2 style={{ color: '#ABFD13', fontWeight: "bold" }}>Mision</h2>} 
                bordered= {false} 
                className="cards-about">
                  <p>Create an unforgettable sports experience with the best facilities available anytime according to your needs. Let's start now to find the ideal field for your next match!</p>
                </Card>
              </div>
              </Col>
            </Row>
            <div style={{paddingTop: "190px", justifyContent: "center", display: "flex", paddingBottom: "50px"}}>
              <h2 className="h2-about">
                Available
                <span style={{ color: "#D9D9D9" }}> For Rent</span>
              </h2>
            </div>
            <div>
            <Row gutter={24}>
              <Col span={8}>
                <Card 
                  hoverable
                  cover={<img alt="example" src="https://os.alipayobjects.com/rmsportal/QBnOOoLaAfKPirc.png" />}
                >
                  <Meta title="Europe Street beat" description="www.instagram.com" />
                </Card>
              </Col>
              <Col span={8}>
                <Card 
                  hoverable
                  cover={<img alt="example" src="https://os.alipayobjects.com/rmsportal/QBnOOoLaAfKPirc.png" />}
                >
                  <Meta title="Europe Street beat" description="www.instagram.com" />
                </Card>
              </Col>
              <Col span={8}>
                <Card 
                  hoverable
                  cover={<img alt="example" src="https://os.alipayobjects.com/rmsportal/QBnOOoLaAfKPirc.png" />}
                >
                  <Meta title="Europe Street beat" description="www.instagram.com" />
                </Card>
              </Col>
            </Row>
            </div>
          </Col>
        </section>

        {/* Contact Section */}
        <section
          id="Contact"
          className="contact-section"
        >
          <Row gutter={24}>
            <Col span={12}>
              <div className="div-contact">
                <h2
                  style={{ fontSize: "2.5rem", fontWeight: "bold", color: "#ABFD13", paddingLeft: "150px", paddingRight: "120px" }}
                >
                  Contact <b style={{ color: "#d9d9d9" }}>Us</b>
                </h2>
                <p style={{ paddingBottom: "50px" ,paddingLeft: "150px", paddingRight: "120px", color: "#d9d9d9", fontSize: "16px"}}>
                  Have questions? We'd love to hear from you. Send us a message and
                  we'll respond as soon as possible.
                </p>
                <ul className="example-2" style={{alignItems: "start", display: "flex", paddingLeft: "140px"}}>
                  <li className="icon-content">
                    <a
                      href="https://linkedin.com/"
                      aria-label="LinkedIn"
                      data-social="linkedin"
                    >
                      <div className="filled" />
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={16}
                        height={16}
                        fill="currentColor"
                        className="bi bi-linkedin"
                        viewBox="0 0 16 16"
                        xmlSpace="preserve"
                      >
                        <path
                          d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"
                          fill="currentColor"
                        />
                      </svg>
                    </a>
                    <div className="tooltip">LinkedIn</div>
                  </li>
                  <li className="icon-content">
                    <a
                      href="https://www.github.com/"
                      aria-label="GitHub"
                      data-social="github"
                    >
                      <div className="filled" />
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={16}
                        height={16}
                        fill="currentColor"
                        className="bi bi-github"
                        viewBox="0 0 16 16"
                        xmlSpace="preserve"
                      >
                        <path
                          d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8"
                          fill="currentColor"
                        />
                      </svg>
                    </a>
                    <div className="tooltip">GitHub</div>
                  </li>
                  <li className="icon-content">
                    <a
                      href="https://www.instagram.com/"
                      aria-label="Instagram"
                      data-social="instagram"
                    >
                      <div className="filled" />
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={16}
                        height={16}
                        fill="currentColor"
                        className="bi bi-instagram"
                        viewBox="0 0 16 16"
                        xmlSpace="preserve"
                      >
                        <path
                          d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334"
                          fill="currentColor"
                        />
                      </svg>
                    </a>
                    <div className="tooltip">Instagram</div>
                  </li>
                  <li className="icon-content">
                    <a
                      href="https://youtube.com/"
                      aria-label="Youtube"
                      data-social="youtube"
                    >
                      <div className="filled" />
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width={16}
                        height={16}
                        fill="currentColor"
                        className="bi bi-youtube"
                        viewBox="0 0 16 16"
                        xmlSpace="preserve"
                      >
                        <path
                          d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z"
                          fill="currentColor"
                        />
                      </svg>
                    </a>
                    <div className="tooltip">Youtube</div>
                  </li>
                </ul>
              </div>
            </Col>
            <Col span={12} style={{paddingLeft: "90px"}}>
              <div style={{fontSize: "16px", fontFamily: "Poppins"}}>
                <p style={{lineHeight: "2", fontSize: "16px"}}>Help Center 
                  <br/>Terms & Conditions
                  <br/>Privacy Policy
                </p>
                <p style={{ lineHeight: "2", fontSize: "16px"}}>
                  <br/>rentfield@gmail.com
                  <br/>(+62) 83111274549
                  <br/>Udayanan street, Singaraja bali
                </p>
              </div>
            </Col>
          </Row>
        </section>
      </main>

      <Footer className="footer-landing">
        <p className="copyright font-color">
          {" "}
          Copyright © 2024 RentField.com - Powered by CodeBlue Universitas Pendidikan
          Ganesha
        </p>
      </Footer>
    </Layout>
  );
}

export default LandingPage;
